<template>
  <div class="layout">
    ...
    <k-table
      ...
      :draggable="true"
    >
      <template slot="xxx" slot-scope="{ row }">
        <a :class="css.code" href="javascript:;">{{ row.Code }}</a>
      </template>
      <b slot="TotalPrice" style="color: red" slot-scope="scope">
        {{ parseFloat(scope.row.TotalPrice).toFixed(2) }}</b
      >
      <ul slot="Detail" :class="css.detail" slot-scope="{ row }">
        <li v-for="item in row.Details" :key="item.Id">
          {{ item.ProName }} &times; {{ item.ProCount }}
        </li>
      </ul>
    </k-table>
    <div>
      <k-button type="warning" @click="onClear">清空所选</k-button>
