  ...
  methods: {
    ...
    getFilterData() {
      if (this.inputText.trim() !== '' && !this.matchFromDatabase) {
        //搜索出来
        let arr = []
        //先整个匹配，如果匹配到了则直接返回，没有匹配到的话，再用其他匹配方式
        const wholeText = this.inputText.trim()
        const nameField = this.valueField
        const result = this.data.filter((item) => {
          return item[nameField].trim() === wholeText
        })
    ...
    init() {
      this.$nextTick(() => {
        ...
        if (this.ins) {
          const { layerWidth, layerHeight, filterData, $slots, $scopedSlots } =
            this
